...
/* harmony default export */ var changePassword = (external_blitz_["resolver"].pipe(external_blitz_["resolver"].zod(validations["a" /* ChangePassword */]), external_blitz_["resolver"].authorize(), async ({
  currentPassword,
  newPassword
}, ctx) => {
  const user = await db["default"].user.findFirst({
  ...
  if (!user) throw new external_blitz_["NotFoundError"]();
  await Object(login["a" /* authenticateUser */])(user.email, currentPassword);
  const hashedPassword = await external_blitz_["SecurePassword"].hash(newPassword);
  await db["default"].user.update({
    where: {
      id: user.id
    },
...
// EXTERNAL MODULE: external "@blitzjs/server"
var server_ = __webpack_require__("kHmN");
...
/* harmony default export */ var auth_mutations_changePassword = __webpack_exports__["default"] = (Object(server_["rpcApiHandler"])(mutations_changePassword, Object(core_["getAllMiddlewareForModule"])(mutations_changePassword), () => {
  var _connect;
